...
func maxArea(height []int) int {
	...
	for left < right {
		...
		if height[left] < height[right] {
			...
			for left+1 < right && height[left+1] <= leftHeight {
				left++
			}
			left++
		} else {
			...
			for left < right-1 && height[right-1] <= rightHeight {
				right--
			}
			right--
		}
...
* 多加了一个“跳过无效高度”的优化（类似“滑动窗口”）。
* **核心想法**是：跳过那些不可能产生更大面积的高度。

**潜在问题：**

* 虽然理论上减少了无意义的比较，但增加了更多判断和循环逻辑。
* 在最坏情况下仍可能遍历大部分元素。
* **运行时不一定更快**：因为 `for` 循环内部可能多次跳过，但判断多了，CPU 分支多了。

---

## 📊 总结对比：

| 项目    | 第一版（标准双指针） | 第二版（跳过优化）            |
| ----- | ---------- | -------------------- |
| 时间复杂度 | O(n)       | O(n) 最坏              |
| 实际效率  | ✅ 更快（多数情况） | ❌ 判断更多，不一定更快         |
| 可读性   | ✅ 简洁       | ❌ 增加复杂性              |
| 推荐程度  | ⭐⭐⭐⭐       | ⭐⭐（不推荐 unless 特别大输入） |
...
在**绝大多数输入情况下，第一版更高效、简洁、性能更稳定**。第二版的“优化”其实带来的收益有限，反而可能拖慢速度，属于“过度优化”。

---

如你在面试、生产中用，**推荐直接使用第一版写法。**
