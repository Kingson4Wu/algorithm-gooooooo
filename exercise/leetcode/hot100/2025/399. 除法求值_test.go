...
func calcEquation(equations [][]string, values []float64, queries [][]string) []float64 {
	...
	chReserveChain := make(map[string][]string)
	chReserveValueChain := make(map[string][]float64)
	...
	for i := 0; i < len(equations); i++ {
		...
		if l, ok := chReserveChain[equations[i][1]]; ok {
			chReserveChain[equations[i][1]] = append(l, equations[i][0])
		} else {
			chReserveChain[equations[i][1]] = []string{equations[i][0]}
		}
		if l, ok := chReserveValueChain[equations[i][1]]; ok {
			chReserveValueChain[equations[i][1]] = append(l, values[i])
		} else {
			chReserveValueChain[equations[i][1]] = []float64{values[i]}
		}
	}

	getValue := func(k, v string, reverse bool) float64 {
		visited := make(map[string]bool)
		var vals []float64
		var result float64 = -1
		var dfs func(a, b string)
		chain := chChain
		valChain := chValueChain
		if reverse {
			chain = chReserveChain
			valChain = chReserveValueChain
		}
		dfs = func(a, b string) {
			if l, ok := chain[a]; ok {
				for j, ch := range l {
	...
	for i := 0; i < len(queries); i++ {
		...
		val := getValue(queries[i][0], queries[i][1], false)
		if val == -1 {
			val = 1 / getValue(queries[i][0], queries[i][1], true)
		}
		ans[i] = val
	}
	return ans
